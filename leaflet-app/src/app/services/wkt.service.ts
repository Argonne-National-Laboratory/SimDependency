...
export class WktService {
  ...
  /**
   * Accepts one geometry from a GeoJson feature set and returns a WKT string
   * @param {GeoJSON} data - A single geometry of a geojson feature
   * @return {String} - Returns an equivalent well known text (WKT)
   * @return {null} - Input not a valid GeoJSON geometry feature
   */
  geometryToWKT(data: any) {
    return (typeof (data.type) == 'undefined' || typeof (data.coordinates) == 'undefined') ? null : this.WKT.stringify(data);
  }

  /**
   * Accepts a WKT string and transforms it into an equivalent GeoJSON geometry
   * @param {String} wktString - String encoding the well known text (WKT)
   * @return {GeoJSON} - Returns a GeoJSON of the WKT
   * @return {null} - wktString not a valid WKT string
   */
  wktToGeometry(wktString: string) {
    return this.WKT.parse(wktString);
  }

  /**
   * Namespace adapted from the jquery.geo library (https://github.com/jQueryGeo/geo) to produce well known text (WKT).
   */
  WKT = (function () {
